...
# @jit(nopython=True)
def semi_supervised_transcribe_cnmf(path, beta, itmax, tol, W_dict, time_limit=None, H0=None, plot=False, model_AD= False, channel = "Sum", num_bins = 4096):
    """
    ...
    # we remove firstly the columns whose contents are less than 1e-10 with a while loop

    while (X[:, 0] < 1e-10).all():
        X = X[:, 1:]

    # then we remove all rows whose contents are less than 1e-10
    # dict_w[10][4096][88]
    # X[4096][1501]
