...
def CalculateEQMask(midi_note, piano_H, piano_W, dictionary, threshold=0.1):
    H_mag_spec = NoteSpectrogram(midi_note, piano_H)

    if type(piano_W) == str:
        W_mag_spec = NoteSpectrogram(midi_note, piano_W)
    else:
        H = np.zeros((88, H_mag_spec.shape[1]))
        H[int(midi_note)-21, :] = np.linspace(1, 0, H_mag_spec.shape[1])
        W_mag_spec = np.sum(np.dot(dictionary[t], MM.shift(H, t)) for t in range(10))
        d = 0

    H_mag_spec /= np.max(H_mag_spec)
    W_mag_spec /= np.max(W_mag_spec)
